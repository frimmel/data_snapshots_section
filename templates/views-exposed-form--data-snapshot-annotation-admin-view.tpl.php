 ...
 */
$query = db_select('field_data_field_dssds_data_product', 'ona');
$query->join('field_data_field_dssds_machine_name', 'mna', '(ona.entity_id = mna.entity_id)');
$query->fields('ona', array('field_dssds_data_product_title'))
  ->fields('mna', array('field_dssds_machine_name_value'))
  ->distinct()
  ->orderBy('field_dssds_data_product_title', 'DESC');
$rows = $query->execute()
  ->fetchAll();
...
foreach ($rows as $row) {
  ...
  $new_form .= '>' . $row->field_dssds_data_product_title . '</option>';
}
$new_form .= '</select>';
$new_form .= '</div>';
